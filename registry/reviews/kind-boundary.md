...
|---|---|---|---|
| `field` vs `status_value` | A status field such as `task_lifecycle_status` has both a field name and allowed values. | `field` registers the slot name; `status_value` registers allowed values for that slot, with the slot/domain in `applies_to`. | Keep separate. |
...
| `field` vs `classification_field` | Categorical field names are structurally fields, but their values represent semantic axes such as type, status, scope, sector, side, or tags. | Use `classification_field` for categorical/classification slots; merge semantically identical axes and put all usage scopes in `applies_to`. Prefer explicit semantic payloads such as `source_event_type`, `trade_side_type`, `impact_scope`, or `source_theme_tags` over vague/source-specific names like `category` or `side_hint`. | Split classification fields. |
...
| `request_type` vs `status_value` | Requests initiate work; task states track lifecycle. | `request_type` classifies requested work; `status_value` rows with `applies_to = task_lifecycle_status` track execution state values. | Keep separate. |
...
```text
plain non-temporal/non-categorical field names go in `field`; date/time field names go in `temporal_field`; categorical field names go in `classification_field`; allowed status/policy values go in `status_value` with the domain in `applies_to`.
```
