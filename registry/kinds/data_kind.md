...
Use `payload` for a stable snake_case data-kind key. For final saved data kinds with accepted templates, use `path` for the canonical source-specific `trading-data/templates/data_kinds/<source>/*.preview.csv` template/preview file. For source-only or transient data kinds without accepted final templates, leave `path` null until a final template exists. Provider/source documentation URLs belong on provider/source `term` rows, not on final `data_kind` rows. Use `applies_to` for the owning source, bundle, or component scope.
