# Templates

`storage/templates/` stores reusable trading-wide templates for docs, contracts, task prompts, manifests, requests, ready signals, and helper usage.

Templates should remain reusable across repositories unless explicitly labeled as component-specific.

See `../../docs/09_templates.md` for the docs-level template operating guide.

## Template Sets

- `contracts/` — reusable contract drafting templates.
- `data_tasks/` — parked draft templates for manager-driven historical `trading-data` acquisition tasks. These are not active registry contracts until the task architecture is redesigned after model/bundle contracts settle.
- `project_development/` — parked project-development slot drafts moved out of the OpenClaw skill. These are not active registry contracts until reviewed one by one and re-registered.
