...
- M06 event-family promotion staging is internal to the M06 attribution run. The accepted artifact set is `event_focus_proposals.jsonl`, `temporal_attention_candidate_pool.jsonl`, `event_family_occurrence_scan.jsonl`, `event_family_bias_association_packets.jsonl`, `event_strategy_promotion_reviews.jsonl`, and `accepted_temporal_attention_pool_entries.jsonl`. Deterministic gates decide co-event/confounder, PIT leakage, matched-control/base-rate, and bias-association status before any agent review. `event-strategy-promotion-review` is a final guard and can only accept already reviewable packets into temporal-attention pool evidence; it does not compute the deterministic gates. Event-family packets must distinguish pre-release uncertainty from post-release observed market impact, including the same event family changing phase after a formal release becomes available point-in-time. Packets may express M03 event-state state overlays such as risk-state shift or uncertainty-risk elevation without claiming a linear up/down forecast.
- M06 event-family modelability uses two program-owned contracts before Codex semantic review. `model_06_event_family_modelability_acquisition_plan` prepares bounded provider task keys through canonical source acquisition routes and the reviewed provider dispatcher. `model_06_event_family_modelability_evidence_packet` groups acquired same-family point-in-time evidence into the packet consumed by Codex review. These stages do not judge projection mode, probability-function class, direction, magnitude, half-life, or utility delta. A single observed event is only a candidate seed; M06 cannot assign an event-family probability-function type until multiple point-in-time same-family observations, coverage evidence, controls, and leakage/overlap checks are available. Codex modelability skills consume the resulting evidence packet and must perform zero provider calls.
- `manager_model_06_post_replay_workflow_plan` is the manager plan surface for the M06 lane after replay review. It uses the same lifecycle stage vocabulary as other model work: data acquisition materializes the full PIT event universe for the replay-reviewed fold, feature generation builds event-family evidence/modelability gate routes, model generation runs residual-event attribution, then evaluation, promotion review, and maintenance handoff follow. This plan does not move M06 into the pre-replay M01-M05 workflow and does not authorize provider calls, broker execution, or model activation.
...
- `docs/31_event_impact_distribution.md` is the canonical M03/M06 event-impact distribution contract. It defines projection modes, probability-function classes, M06 modelability responsibilities, M03 event-state projection responsibilities, and the program/agent boundary.
