...
- OKX is registered as a provider term for crypto data acquisition and trading. Registry config row `OKX_SECRET_ALIAS` points to source-level alias `okx` and path `/root/secrets/okx.json`; JSON fields are `api_key`, `secret_key`, and `passphrase`. Non-secret metadata remains separate: allowed IPv4 `66.206.20.138` and API key remark `OpenClaw`.
- Source secrets use one JSON file per source/provider under `/root/secrets/<source>.json`; reusable JSON key names such as `api_key`, `secret_key`, `passphrase`, and `pat` are registered as `field` rows with `applies_to=source_secret_json`.
