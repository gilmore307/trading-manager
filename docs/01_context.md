...
- Provider calls, storage lifecycle mutation, model activation, and broker/account actions are separate gates.
...
```text
manager request -> component execution -> completion receipt -> run/artifact/ready rows -> summary/review/promotion gate
```
