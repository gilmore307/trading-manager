...
- Extend the implemented persistent scheduler runtime beyond safe Layer 1 preparation into internal approval-gated provider acquisition, receipt ingestion, ready-signal progression, offline feature/model/evaluation dispatch, and promotion-review preparation.
...
- Implemented first scheduler tick and persistent runtime: `scripts/tasks/run_automation_scheduler.py` evaluates market-day/time and resource gates, and `scripts/tasks/run_automation_scheduler_daemon.py` provides checkpointed resident operation with lock/state/decision log support. Both preserve zero provider calls until approval-gated acquisition is implemented.
