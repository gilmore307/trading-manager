...
- Implemented first scheduler tick, persistent runtime, full Layer 1-8 workflow graph, durable workflow checkpoint, the first approval-gated provider dispatch adapter, and the safe offline stage executor: `scripts/tasks/run_automation_scheduler.py` evaluates market-day/time and resource gates; `scripts/tasks/run_automation_scheduler_daemon.py` provides checkpointed resident operation with lock/state/decision log support; `scripts/tasks/plan_model_training_workflow.py` exposes the data-acquisition/feature/model/evaluation/review/maintenance graph for all eight layers; `scripts/tasks/advance_model_training_workflow.py` ingests receipts/approval refs and advances stage state; `scripts/tasks/dispatch_approved_provider_acquisition.py` validates Layer 1 `live_call_approval_v1` and only dispatches provider calls with the explicit execute flag; `scripts/tasks/execute_model_training_stage.py` runs one ready safe offline stage and writes logs/receipts while refusing approval-gated provider stages.
