...
Progress stall guard: the daemon uses `TRADING_MANAGER_SCHEDULER_PROGRESS_STALL_SECONDS=600` by default. If no scheduler progress is observed for that window, it writes a `scheduler_progress_stalled` server-error handoff and invokes the configured agent repair runner. Executed decisions count as progress, and replay option-feature drain batches that remain in `backoff` while performing bounded provider/source/feature work count as progress until the next replay retry is eligible. If a separate bounded replay option-feature drain already owns the replay option-feature lock, the daemon records a wait/backoff decision instead of converting that coordination state into a scheduler iteration error. The registry row `TRADING_MANAGER_SCHEDULER_PROGRESS_NONPROGRESS_REASONS` owns daemon state reason values that are known lifecycle waits, active replay option-feature drain ownership, or completed bounded model-group work rather than stalls. Stage subprocesses use `TRADING_MANAGER_STAGE_PROGRESS_STALL_SECONDS=600` by default and must keep their active task-progress file fresh while running.

Long-running fold-scoped model-generation stages must publish two progress
scales while running: row-level heartbeat counters for liveness and month-level
`month_progress` for owner tracking. `month_progress` reports the current
chronological dataset month plus completed/expected dataset-month counts for the
active train/validation/test split. Rows may update many times inside one month,
but the month count must advance when generation crosses into the next dataset
month and must close at the full split count when the stage completes.

Stage resource guard: safe offline stage commands must be restartable and bounded. The executor enforces `TRADING_MANAGER_STAGE_MAX_RSS_MB=16384` as a process-tree RSS cap and `TRADING_MANAGER_STAGE_MIN_AVAILABLE_MEMORY_MB=4096` as a host reserve. If either limit is crossed, the executor terminates the whole stage process group, records `stage_resource_guard_triggered`, and routes the failure through automatic repair. Large database/model stages must stream or batch their input instead of materializing all rows in memory; the guard is the last safety layer, not a license to allocate the whole dataset. The checked-in service template also sets `MemoryHigh=20G`, `MemoryMax=24G`, `MemorySwapMax=1G`, `OOMPolicy=stop`, and `KillMode=control-group` so a broken historical task cannot starve SSH, OpenClaw, or the rest of the host.
