...
Event-family packets do not have to prove a linear directional relationship. Layer 10 separates pre-release uncertainty from post-release observed impact. Before formal release, earnings, filings, guidance, and scheduled macro releases usually describe elevated uncertainty or risk around the target, not a prediction of the result. After the release is available point-in-time, the same event family becomes observed market-impact evidence. Accepted packets hand Layer 4 a phase-aware state overlay such as `event_risk_state_shift` or `event_uncertainty_risk_elevated` for downstream state composition.
