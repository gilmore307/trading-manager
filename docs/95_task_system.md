...
A task-level `ready_signal_v1` is not enough to unlock a workflow stage. Stage advancement must pass a `manager_stage_coverage_v1` gate over `task_summary`: for example, Layer 1 January 2016 data acquisition remains `partial_ready` at `3/22` ready requests and may unlock feature generation only at full expected coverage (`22/22`) with `can_unlock_downstream=true`. Full coverage means every expected request has terminal, reviewed evidence; it does not mean every symbol produced positive rows. A zero-row receipt with explicit `no_data_response` / not-yet-listed evidence may count as ready coverage, while provider/schema failures remain failed until corrected or explicitly reclassified by a reviewed policy.
