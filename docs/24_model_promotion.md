...
are sourced from the point-in-time interpreted event/effect ledger, one logical
event/effect row per row, and must not inherit target symbols from M02 or
selected trades from M04-M05.
