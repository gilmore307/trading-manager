...
| 4 | `model_05_alpha_confidence` | `AlphaConfidenceModel` | `alpha_confidence_vector` | current `model_05_alpha_confidence` evidence and `4_*` score tokens |
| 5 | `model_06_position_projection` | `PositionProjectionModel` | `position_projection_vector` | current `model_06_position_projection` evidence and `5_*` score tokens |
| 6 | `model_07_underlying_action` | `UnderlyingActionModel` | `underlying_action_plan` | current `model_07_underlying_action` evidence and `6_*` score tokens |
| 7 | `model_08_option_expression` | `OptionExpressionModel` under `TradingGuidanceModel` | `option_expression_plan` | current `model_08_option_expression` evidence and `7_*` score tokens |
| 8 | `model_09_event_risk_governor` | `EventRiskGovernor` | `event_context_vector` / `event_risk_intervention` | registered event-context score-family tokens |
