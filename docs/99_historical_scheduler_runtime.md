...
The daemon now carries a manager-owned `manager_model_training_workflow_plan` for the base conceptual Layers 1-8 plus durable workflow checkpoints. Event intelligence is the separate conceptual Layer 9 risk-governor overlay and is not a hard dependency for base Layer 1-8 progression. Month-scoped checkpoints expose only reusable substrate stages: data acquisition and feature/input preparation. Model generation, model evaluation, Promotion Review, and maintenance are fold-scoped `Model Worker 1` tasks that consume frozen 4+1+1 manifests, not per-month scheduler stages. Conceptual Layer 4 EventFailureRiskModel is governance/contract-only in this slice; legacy physical Layers 4-6 intentionally mark trading-data feature generation as `not_applicable` because their inputs are upstream model/control-plane/position-risk artifacts rather than new provider data surfaces; conceptual Layer 8 uses the legacy option-expression adapter only after the Layer 7 underlying-action chain is ready.
