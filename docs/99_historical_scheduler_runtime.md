...
The daemon now carries a manager-owned `manager_model_training_workflow_plan` for the base seven model layers plus durable workflow checkpoints. Event intelligence is the separate conceptual Layer 8 risk-governor overlay and is not a hard dependency for base Layer 1-7 progression. Month-scoped checkpoints expose only reusable substrate stages: data acquisition and feature/input preparation. Model generation, model evaluation, Promotion Review, and maintenance are fold-scoped `Model Worker 1` tasks that consume frozen 4+1+1 manifests, not per-month scheduler stages. Base Layers 4-6 intentionally mark trading-data feature generation as `not_applicable` because their inputs are upstream model/control-plane/position-risk artifacts rather than new provider data surfaces; base Layer 7 uses the legacy option-expression adapter only after the Layer 6 chain is ready.
...
| Event risk governor / Layer 8 overlay | Separate event-intelligence overlay that may later review base Layer 7 guidance; it is bypassed for base workflow progression and must remain auditable side by side with the base decision. |
