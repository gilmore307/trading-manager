...
This module owns the safe post-dispatch side of an approval-gated provider
stage. It discovers component completion receipts that already exist on disk,
...
def discover_stage_receipts(
    *,
    ...
    for member in load_market_regime_universe(model_layers=(model_layer,)):
        ...
        receipt_path = component_storage_root / "monthly_backfill_v1" / "alpaca_bars" / symbol / start_month / "completion_receipt.json"
    ...
    for ref in refs:
        ...
        normalized.append(
            normalize_completion_receipt(
                ...
                consumer_hint="manager_provider_stage_reconcile_v1",
            )
        )
                ...
                validate_failure_register_row(
                    {
                        ...
                        "note": "Generated by manager_provider_stage_reconcile_v1 from a failed component receipt; requires agent review before correction, retry, or accepted skip.",
                    }
                )
            )
    return tuple(rows)
