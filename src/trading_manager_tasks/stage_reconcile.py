...
def classify_provider_failure(error_summary: str) -> tuple[str, str, str]:
    ...
    return (
        ...
        "Generated by manager_provider_stage_reconcile from a latest failed component receipt; requires agent review before correction, retry, or accepted skip.",
    )
...
def propose_failure_register_rows(
    ...
    *,
    ...
    contract, or unclassified errors remain `agent_review_required`. These rows
...
def _reconcile_provider_stage_unlocked(
    *,
    ...
    agent_review_failure_rows = tuple(
        row for row in failure_rows if str(row.get("failure_status") or "") == "agent_review_required"
    )
    ...
    agent_error_result = (
        _handle_stage_failure_agent_handoff(
            ...
            start_month=start_month,
            end_month=end_month,
            rows=agent_review_failure_rows,
            proposal_path=proposal_path,
            database_url=database_url,
        )
        if persist_failure_register and agent_review_failure_rows
        else None
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--write-failure-register", action="store_true", help="Persist proposed failure-register rows; retryable provider/runtime failures become retry_required, while unclassified failures require agent review.")
