...
from .residual_event_governance_inputs import materialize_residual_event_governance_inputs_inputs
...
from .workflow_transition_ledger import append_transition, append_work_selection_transition
...
ROLLING_FOLD_MONTH_COUNT = ROLLING_FOLD_SIZE_MONTHS
ROLLING_FOLD_STEP_MONTH_COUNT = ROLLING_FOLD_STEP_MONTHS
...
def completed_historical_fold_cutoff_month(max_completed_month: str) -> str:
    """Return latest completed month available for fold-scoped training selection."""

    _year_text, month_text = max_completed_month.split("-", 1)
    month_number = int(month_text)
    if month_number < 1 or month_number > 12:
        raise ValueError(f"invalid month: {max_completed_month}")
    return max_completed_month
...
def completed_historical_fold_cutoff(now: datetime | None = None) -> str:
    ...
    Historical training is scheduled by complete 12+3+3 walk-forward windows. A
    new fold does not open until its final test-window month is complete in the
    project timezone.
    """
...
def select_next_historical_work(
    *,
    ...
    open_target_fold = (
        _open_model_worker_fold_for_target(
    ...
    if ready_target_fold is not None:
        return HistoricalWorkSelection(
            start_month=ready_target_fold.start_month,
            end_month=ready_target_fold.end_month,
            reason_code=ready_target_fold.reason_code,
            completed_months=completed_tuple,
            open_months=open_tuple,
            blocked_fold_start_month=(
                ready_target_fold.start_month if ready_target_fold.reason_code == "blocked_model_worker_fold_holds_target_lane" else None
            ),
            blocked_fold_end_month=(
                ready_target_fold.end_month if ready_target_fold.reason_code == "blocked_model_worker_fold_holds_target_lane" else None
            ),
            blocked_fold_state_path=(
                ready_target_fold.state_path if ready_target_fold.reason_code == "blocked_model_worker_fold_holds_target_lane" else None
            ),
...
def select_month_ingest_worker_months(
    *,
    ...
    if active_target_symbol and target_has_open_model_worker_fold(
        storage_root=storage_root,
        selected_target_symbol=active_target_symbol,
        max_month=max_month,
    ):
        return ()
    if active_target_symbol and model_group_lifecycle_blocks_next_fold(
        storage_root=storage_root,
        selected_target_symbol=active_target_symbol,
    ):
        return ()
    if active_target_symbol and select_model_worker_fold(
        storage_root=storage_root,
        default_start_month=default_start_month,
        max_month=max_month,
        selected_target_symbol=active_target_symbol,
...
def rolling_fold_months(start_month: str, *, month_count: int = ROLLING_FOLD_MONTH_COUNT) -> tuple[str, ...]:
    """Return the inclusive month sequence for one frozen 12+3+3 fold."""
...
def _is_current_rolling_fold_span(start_month: str, end_month: str) -> bool:
    """Return whether a persisted fold state matches the current walk-forward fold contract."""
...
def _advance_fold_start_month(start_month: str, *, month_count: int = ROLLING_FOLD_STEP_MONTH_COUNT) -> str:
    """Return the next cumulative walk-forward fold start month."""
...
def select_model_worker_fold(
    *,
    ...
    """Select the earliest complete 12+3+3 walk-forward fold with open Model Worker work."""
...
def active_model_worker_target_symbol(
    *,
    ...
    if requested and requested in target_queue:
        return requested
    return target_queue[0] if target_queue else None


def _append_selection_transition_if_needed(
    *,
    selection: HistoricalWorkSelection,
    selected_target_symbol: str | None,
    decision_log_path: Path,
    previous_state: SchedulerDaemonState,
    next_state: SchedulerDaemonState,
) -> None:
    """Record lane selection in the canonical workflow ledger when it changes."""

    scope_changed = previous_state.start_month != next_state.start_month or previous_state.end_month != next_state.end_month
    selection_changed = (
        previous_state.last_work_selection_reason != next_state.last_work_selection_reason
        or previous_state.selected_target_symbol != next_state.selected_target_symbol
        or previous_state.last_completed_months != next_state.last_completed_months
        or previous_state.last_open_months != next_state.last_open_months
    )
    if not scope_changed and not selection_changed:
        return
    append_work_selection_transition(
        selection.summary_row(),
        selected_target_symbol=selected_target_symbol or next_state.selected_target_symbol,
...
def apply_auto_work_selection_with_ledger(
    ...
    *,
    ...
    """Align daemon scope and persist the resulting lane selection transition."""

    legal_target = active_model_worker_target_symbol(
        requested_target_symbol=state.selected_target_symbol,
        target_queue_path=target_queue_path,
    )
    selection = select_next_historical_work(
        ...
        selected_target_symbol=legal_target,
        target_queue_path=target_queue_path,
    )
    next_state = apply_auto_work_selection(
        replace(state, selected_target_symbol=legal_target),
        storage_root=storage_root,
        default_start_month=default_start_month,
        default_end_month=default_end_month,
        target_queue_path=target_queue_path,
    )
    _append_selection_transition_if_needed(
        selection=selection,
        selected_target_symbol=legal_target or selection.blocked_target_symbol,
    ...
    *,
    ...
    if not target_queue:
        return ModelWorkerTargetSelection(
            selected_target_symbol=None,
            target_queue=(),
            reason_code="no_model_worker_target_queue_available",
            fold_selection=None,
        )
...
def apply_auto_work_selection(
    ...
    *,
    ...
    selection_changed = (
        ...
        or state.selected_target_symbol != active_target_symbol
    )
    if not scope_changed and not selection_changed:
        return state
    return replace(
        ...
        selected_target_symbol=active_target_symbol or selection.blocked_target_symbol,
...
def _compact_task_key_summary(summary: Any, *, sample_limit: int = 5) -> dict[str, Any]:
    row = summary.summary_row() if hasattr(summary, "summary_row") else dict(summary or {})
    task_keys = row.get("task_keys")
    if isinstance(task_keys, list) and len(task_keys) > sample_limit:
        row["task_key_count"] = len(task_keys)
        row["task_keys_sample"] = task_keys[:sample_limit]
        row.pop("task_keys", None)
    return row
...
def _m06_event_input_decision(
    *,
    ...
    return SchedulerDecision(
        ...
        lock_plan=scheduler_lock_plan(month=None, selected_work="model_group.m06_event_inputs", next_internal_stage="model_group.m06_event_inputs"),
    )


def _m06_event_input_start_decision(
    residual_event_decision: SchedulerDecision,
    *,
    selected_target_symbol: str | None = None,
) -> SchedulerDecision | None:
    if residual_event_decision.reason_code not in M06_EVENT_INPUT_MISSING_REASONS:
        return None
    ...
    return _m06_event_input_decision(
        decision_status="backoff",
        reason_code="model_group_m06_event_feed_backfill_running",
        reason=(
            f"M06 attribution is waiting for event inputs; preparing and dispatching bounded event-feed backfill "
...
def _run_m06_event_input_requirement_handoff(
    residual_event_decision: SchedulerDecision,
    *,
    storage_root: Path,
    execute: bool,
    ...
    if not execute:
        return _m06_event_input_decision(
            decision_status="backoff",
            reason_code="model_group_m06_event_input_preparation_required",
            reason="M06 event inputs are missing; event-feed task keys must be prepared before attribution",
            command=command,
            provider_calls=0,
            dispatch_performed=False,
            execution_summary=execution_summary,
    ...
    if not execute_provider_acquisition:
        execution_summary["required_next_step"] = "enable autonomous provider acquisition to backfill M06 event feeds before materialization"
        return _m06_event_input_decision(
            decision_status="backoff",
    ...
    if failed_items:
        ...
        return _m06_event_input_decision(
            decision_status="backoff",
            reason_code="model_group_m06_event_feed_backfill_failed",
            reason="one or more M06 event-feed backfill requests failed; repair provider/source errors before attribution",
            command=command,
            provider_calls=provider_calls,
            dispatch_performed=dispatch_performed,
            execution_summary=execution_summary,
    ...
    materialization_probe = materialize_residual_event_governance_inputs_inputs(
        start_month=start_month,
        end_month=end_month,
    ...
    if missing_coverage or missing_rows:
        ...
        if provider_calls > 0:
            execution_summary["required_next_step"] = "continue bounded M06 event-feed backfill before materializing event inputs"
            return _m06_event_input_decision(
                decision_status="executed",
                reason_code="model_group_m06_event_feed_backfill_executed",
                reason=f"M06 event-feed backfill dispatched {provider_calls} provider request(s); more event input coverage is still required",
                command=command,
                provider_calls=provider_calls,
                dispatch_performed=dispatch_performed,
                execution_summary=execution_summary,
            )
        execution_summary["required_next_step"] = "repair zero-row or missing reviewed event-feed coverage before M06 attribution"
        return _m06_event_input_decision(
            decision_status="backoff",
            reason_code="model_group_m06_event_inputs_incomplete",
            reason="M06 event-feed receipts exist but coverage is still incomplete or zero-row for required sources",
            command=command,
            provider_calls=0,
            dispatch_performed=False,
            execution_summary=execution_summary,
    ...
    materialization = materialize_residual_event_governance_inputs_inputs(
        start_month=start_month,
        end_month=end_month,
        manager_storage_root=storage_root,
        write=True,
    ...
    execution_summary["required_next_step"] = "resume model_group.residual_event_governance attribution"
    return _m06_event_input_decision(
        decision_status="executed",
        reason_code="model_group_m06_event_inputs_materialized",
        reason=f"M06 event inputs are materialized for {start_month} to {end_month}; residual event attribution can resume",
        command=command,
        provider_calls=provider_calls,
        dispatch_performed=dispatch_performed,
        execution_summary=execution_summary,
                    ...
                    if use_single_month_work_loop:
                        ...
                        if residual_event_governance_decision is not None:
                            ...
                            if m06_event_input_start_decision is not None:
                                ...
                                completed = utc_now_iso()
                                state = update_state_from_decision(state, started_utc=started, completed_utc=completed, decision=m06_event_input_start_decision)
                                state = replace(
                                    state,
                                    start_month=active_start_month,
                                    end_month=active_end_month,
                                    last_next_internal_stage="model_group.m06_event_inputs",
                                    last_work_selection_reason="model_group_m06_event_inputs_required",
                                    updated_utc=completed,
                                )
                                should_continue_drain = should_continue_drain or _decision_should_continue_drain(m06_event_input_start_decision, advanced_month=False)
                                decisions_this_cycle += 1
                                if output is not None:
                                    row = m06_event_input_start_decision.summary_row()
                                    row["worker_id"] = "m06_event_input_worker_1"
                                    output.write(json.dumps(row, sort_keys=True) + "\n")
                                    output.flush()
                            m06_event_input_decision = _run_m06_event_input_requirement_handoff(
                                residual_event_governance_decision,
                                storage_root=storage_root,
                                execute=execute_model_group_attribution,
                                execute_provider_acquisition=execute_autonomous_provider_stages,
                            ...
                            if decision_to_record is not None:
                                ...
                                state = replace(
                                    ...
                                    last_next_internal_stage=(
                                        ...
                                        if m06_event_input_decision is not None
                        ...
                        if evaluation_decision is not None:
                            ...
                            decisions_this_cycle += 1
                            if output is not None:
                                row = evaluation_decision.summary_row()
                                row["worker_id"] = "evaluation_worker_1"
                                output.write(json.dumps(row, sort_keys=True) + "\n")
                                output.flush()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--target-symbol", help="Required task-scope target symbol for M03+ 12+3+3 dataset units.")
