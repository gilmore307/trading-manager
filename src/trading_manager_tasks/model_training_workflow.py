...
ROLLING_FOLD_TRAIN_MONTHS = 12
ROLLING_FOLD_VALIDATION_MONTHS = 3
ROLLING_FOLD_TEST_MONTHS = 3
ROLLING_FOLD_SIZE_MONTHS = ROLLING_FOLD_TRAIN_MONTHS + ROLLING_FOLD_VALIDATION_MONTHS + ROLLING_FOLD_TEST_MONTHS
ROLLING_FOLD_STEP_MONTHS = 12
DATASET_UNIT_MONTHS = ROLLING_FOLD_SIZE_MONTHS
ROLLING_FOLD_SPLIT_POLICY = "chronological_cumulative_walk_forward_12_3_3"
...
LAYER_METADATA: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "candidate_axis": "target_symbol;walk_forward_12_3_3_window;target_candidate_id",
    ...
    },
    {
        ...
        "candidate_axis": "target_symbol;walk_forward_12_3_3_window;target_candidate_id;decision_context_id",
    ...
    },
    {
        ...
        "candidate_axis": "target_symbol;walk_forward_12_3_3_window;minute_timestamp;option_contract_bucket",
...
def _dataset_unit_for_layer(
    *,
    ...
    if layer == 1:
        return DatasetUnit(
            unit_kind="walk_forward_12_3_3_panel",
            unit_months=DATASET_UNIT_MONTHS,
            start_month=start_month,
            end_month=end_month,
            target_symbol=None,
            target_required=False,
            description="M01 dataset unit: fixed background panel over one 12+3+3 evaluation window; no single target symbol applies.",
        )
    ...
    return DatasetUnit(
        ...
        description=f"M{layer:02d} dataset unit: target {target_text} over one 12+3+3 evaluation window.",
    )
...
def _rolling_fold_dataset_splits(start_month: str, end_month: str) -> tuple[dict[str, Any], ...]:
    """Return the accepted chronological 12/3/3 split contract for a cumulative walk-forward fold."""
    ...
    if dataset_splits:
        ...
        stages.append(
            WorkflowStage(
                ...
                command=generate,
                dataset_unit=dataset_unit,
                blockers=("rolling_fold_12_3_3_split_required",),
            )
        )
    return LayerWorkflow(
        layer=layer,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--target-symbol", help="Required task-scope target symbol for M02+ 12+3+3 dataset units.")
    parser.add_argument(
