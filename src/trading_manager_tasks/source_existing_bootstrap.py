...
def _event_source_coverage(*, month: str, row_count: int) -> EventSourceCoverage:
    ...
    reason = (
        f"existing model_03_event_state_data_acquisition rows found for {month}; M03 event-state compatibility route can reuse source evidence"
        if row_count > 0
        else f"no model_03_event_state_data_acquisition rows found for {month}; M03 event-state compatibility route has no existing source evidence"
    )
    return EventSourceCoverage(
        contract_type="manager_source_existing_event_coverage",
        source_table="trading_data.model_03_event_state_data_acquisition",
        month=month,
    ...
    *,
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as connection:
        with connection.cursor() as cursor:
            ...
            if table_exists(cursor, "trading_data.model_03_event_state_data_acquisition"):
                cursor.execute(
                    f"""
                    SELECT
                      to_char(date_trunc('month', event_time AT TIME ZONE %s), 'YYYY-MM') AS month,
                      count(*)::BIGINT AS row_count
                    FROM trading_data.model_03_event_state_data_acquisition
                ...
                warnings.append("missing table trading_data.model_03_event_state_data_acquisition")
