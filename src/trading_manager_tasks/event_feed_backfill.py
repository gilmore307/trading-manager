...
DEFAULT_REQUESTED_BY = "trading-manager.layer_ten_event_feed_backfill"
...
def _base_request(feed_id: str, window: MonthlyWindow, *, target_symbol: str) -> dict[str, Any]:
    ...
    return {
        ...
        "availability_note": "Layer 10 event-risk source coverage repair; reviewed artifact required before downstream rebuild.",
    }
...
def prepare_event_feed_backfill(
    *,
    ...
    return EventFeedBackfillSummary(
        contract_type="manager_layer_10_event_feed_backfill_preparation",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prepare Layer 10 event-feed backfill task keys without provider calls.")
