"""Layer 8 option-expression feature-stage adapter.

The Layer 8 option-expression feature stage has two valid paths:
...
class LayerEightFeatureStageSummary:
    """Result for the Layer 8 option-expression feature-generation adapter."""
...
def execute_layer_eight_feature_stage(
    *,
    ...
    """Execute Layer 8 option-expression feature generation through the correct reviewed path."""
    ...
    if review.get("status") == "no_provider_skip_accepted" and int(review.get("active_request_count") or 0) == 0:
        ...
        return LayerEightFeatureStageSummary(
            contract_type="manager_layer_08_option_expression_feature_generation_stage",
            ...
            reason="no active Layer 7 target chain; feature generation is a reviewed no-op",
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Execute Layer 8 option-expression feature generation with reviewed no-provider skip support.")
