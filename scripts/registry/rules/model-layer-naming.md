...
For accepted model state outputs, a reviewed state-vector payload token belongs in `state_vector_value`, not in duplicate `field` and `state_vector_value` rows. `state_vector_value` keys must use `MODEL_NN_<TOKEN>_VALUE`, and payloads must use the owning compact numeric layer prefix (`1_*`, `2_*`, `3_*`). Use `field` only for a distinct storage/schema slot whose reviewed role is physical carriage rather than the state-vector semantic token itself.
...
Shared TargetStateVector V1 state-vector values are registry-owned once both `trading-data` and `trading-model` depend on them. Current shared block payloads use `3_market_state_features`, `3_sector_state_features`, `3_target_state_features`, and `3_cross_state_features`; current sparse synchronized state-window payloads use `3_state_window_5min`, `3_state_window_15min`, `3_state_window_60min`, and `3_state_window_390min`; and accepted Layer 3 scalar score families use compact `3_*` names such as `3_target_direction_score_<window>` and `3_tradability_score_<window>`.
