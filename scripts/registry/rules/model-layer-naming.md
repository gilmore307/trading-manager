...
Shared TargetStateVector V1 row keys and top-level feature-block names are registry-owned once both `trading-data` and `trading-model` depend on them. Registry `state_vector_value.payload` entries carry the owning model number: current shared Layer 3 block names are `3_market_state_features`, `3_sector_state_features`, `3_target_state_features`, and `3_cross_state_features`; the current sparse synchronized state-window values are `3_5min`, `3_15min`, `3_60min`, and `3_390min`; and the accepted sync-policy value is `3_market_sector_target_blocks_must_share_identical_observation_windows`.
