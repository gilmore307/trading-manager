...
CRYPTO_SPOT_ASSET_CLASSES = {"crypto_spot", "spot_crypto", "crypto"}
...
def _mapping_targets(mapping_csv: Path, *, include_crypto_spot: bool) -> list[dict[str, object]]:
    ...
    with mapping_csv.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            if asset_class in CRYPTO_SPOT_ASSET_CLASSES and not include_crypto_spot:
                continue
...
def _option_capability(asset_class: str) -> str:
    if asset_class in CRYPTO_SPOT_ASSET_CLASSES:
...
def build_target_queue(
    *,
    ...
    include_crypto_spot: bool = False,
) -> dict[str, object]:
    mapping_targets = _mapping_targets(mapping_csv, include_crypto_spot=include_crypto_spot)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--include-crypto-spot",
        action="store_true",
        help="Include crypto spot targets after the crypto target-local feed route is verified.",
    )
...
def main() -> int:
    ...
    payload = build_target_queue(
        ...
        include_crypto_spot=args.include_crypto_spot,
    )
