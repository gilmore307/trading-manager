#!/usr/bin/env python3
"""Run Layer 10 event-focus proposal generation when ready."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="write a fresh proposal run even when one already exists for this attribution")
    parser.add_argument("--max-proposals", type=int, default=200)
    args = parser.parse_args(argv)

    decision = run_model_group_event_focus_proposal_if_ready(
    ...
    if decision is None:
        print(json.dumps({"status": "not_ready", "reason_code": "model_group_event_focus_proposal_not_ready"}, sort_keys=True))
        return 0
