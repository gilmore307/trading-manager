...
- If a new `kind` is introduced, update both the SQL kind check and the corresponding `scripts/kinds/<kind>.md` boundary file. The SQL kind check must stay aligned with `scripts/kinds/*.md`; tests enforce this.
...
`trading_registry.payload_format` is constrained to registered value-format markers. When adding a new payload format, update the SQL check constraint, add the matching `kind=payload_format` registry row, update docs/tests, and regenerate `scripts/current.csv` together.
