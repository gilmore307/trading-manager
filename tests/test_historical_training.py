...
class HistoricalTrainingPreparationTests(unittest.TestCase):
    ...
    def test_layer_one_batch_preparation_writes_payloads_and_validates_handoffs(self):
        ...
        self.assertEqual(summary.model_layer, "model_01_market_context")
    ...
    def test_layer_two_batch_preparation_uses_sector_context_universe(self):
        ...
        self.assertEqual(summary.model_layer, "model_01_sector_context")
        self.assertEqual(summary.request_count, 12)
        self.assertEqual(summary.payload_count, 12)
        self.assertEqual(summary.handoff_validation_count, 12)
        self.assertIn("XLK", summary.symbols)
        self.assertIn("BKCH", summary.symbols)
        self.assertNotIn("SPY", summary.symbols)
        self.assertTrue(all(row["model_layer"] == "model_01_sector_context" for row in requests))
        self.assertTrue(all(row["timeframe"] == "1Min" for row in requests))
