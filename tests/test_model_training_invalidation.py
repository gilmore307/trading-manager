...
class ModelTrainingInvalidationTests(unittest.TestCase):
    def test_invalidates_layer_nine_event_risk_outputs_while_preserving_layers_one_three(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
                ...
                json.dumps(
                    {
                        ...
                        "stages": [
                            ...
                            {"stage_id": "layer_09_event_risk_governor.model_generation", "layer": 9, "status": "succeeded", "artifact_refs": ["old"]},
                        ],
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(by_stage["layer_08_option_expression.model_evaluation"]["status"], "ready")
            self.assertEqual(by_stage["layer_09_event_risk_governor.model_generation"]["status"], "failed")
            self.assertIn("rebuild_from_layer_09_event_risk_required", by_stage["layer_09_event_risk_governor.model_generation"]["last_reason"])
            self.assertIn("manager://stale_downstream_from_layer_09_event_source_rebuild_required", by_stage["layer_09_event_risk_governor.model_generation"]["artifact_refs"])
    ...
    def test_dry_run_does_not_write(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            runtime.mkdir()
            state_path = runtime / "model_training_fold_state_2016-01_2016-06.json"
            original = {"stages": [{"stage_id": "layer_09_event_risk_governor.data_acquisition", "layer": 9, "status": "succeeded", "artifact_refs": []}]}
