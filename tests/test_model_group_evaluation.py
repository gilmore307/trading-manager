...
class ModelGroupEvaluationTests(unittest.TestCase):
    ...
    def _write_completed_pre_replay_fold_with_stale_m06_generation(self, storage_root: Path) -> None:
        state_path = storage_root / "runtime" / "model_training_fold_state_aapl_2016-01_2017-06.json"
        state_path.parent.mkdir(parents=True)
        stages = []
        for layer in range(1, 6):
    ...
    def test_evaluation_ignores_stale_pre_replay_m06_generation_fixture(self):
        with tempfile.TemporaryDirectory() as raw_tmp:
            tmp = Path(raw_tmp)
            storage_root = tmp / "storage" / "02_control_plane"
            storage_root.mkdir(parents=True)
            self._write_ready_replay_and_attribution(storage_root)
            self._write_completed_pre_replay_fold_with_stale_m06_generation(storage_root)
