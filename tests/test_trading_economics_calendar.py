...
class TradingEconomicsCalendarPlanningTests(unittest.TestCase):
    ...
    def test_recent_poll_plans_logged_out_recent_feed_task(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(payload["manager_controls"]["max_requests"], 4)
            self.assertEqual(payload["manager_controls"]["retry_policy_ref"], "te_recent_release_fetch_retry_10s_three_attempts_then_websearch")
            self.assertEqual(payload["manager_controls"]["fallback_provider_refs"], ["websearch_public_macro_release"])
            self.assertIn("retry_10s_three_attempts_on_fetch_failure", payload["policy_refs"])
