...
class ModelPromotionRequestTests(unittest.TestCase):
    def test_all_model_readiness_use_one_request_kind(self):
        self.assertEqual(len(MODEL_PROMOTION_TARGETS), 5)
    ...
    def test_builds_valid_manager_request_for_any_model_layer(self):
        ...
        self.assertIn("evaluation_promotion_readiness_policy", request["policy_refs"])
        self.assertIn("execution_runtime_model_lifecycle_policy", request["policy_refs"])
        self.assertNotIn("evaluation_promotion_activation_policy", request["policy_refs"])
