    ...
    def test_frozen_manifest_is_fold_agnostic_after_latest_completed_fold_changes(self):
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            (dataset_root / "base_context.json").write_text(
                json.dumps({"candidate_fold_id": "fold_aapl_2016", "pre_replay_target_refs": ["AAPL"]}) + "\n",
                encoding="utf-8",
            )
            ...
            base_context = json.loads((dataset_root / "base_context.json").read_text(encoding="utf-8"))
            self.assertEqual(manifest["candidate_fold_id"], "fold_aapl_2016")
            self.assertEqual(base_context["candidate_fold_id"], "fold_aapl_2016")
